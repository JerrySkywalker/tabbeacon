...
CHILD_PRETOOLUSE_SUBAGENT_IDENTITY_OBSERVED=true
CHILD_POSTTOOLUSE_SUBAGENT_IDENTITY_OBSERVED=true
...
at least one of `agent_id` or `agent_type` was observed and that normalization
returned `IgnoreSubagent`; record both field presences when Codex supplies both.
Lifecycle-event identity cannot satisfy this generic-event proof. Record only
counts, identity-field presence, and normalization outcomes; never retain a raw
child payload or tool content.
