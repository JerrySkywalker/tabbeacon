...
impl CodexIntegration {
    ...
    pub fn doctor(&self) -> CodexDoctorReport {
        ...
        let manifest_has_known_owned_declarations = manifest
            .as_ref()
            .is_some_and(|manifest| self.manifest_has_known_owned_declarations(manifest));
    ...
    fn manifest_has_known_owned_declarations(&self, manifest: &IntegrationManifest) -> bool {
        owned_command_hooks(&manifest.executable, 1, false)
            ...
            if handlers.iter().any(|handler| {
                !handler.is_object()
                    || !handler
                        ...
                        .is_some_and(|handler_type| !handler_type.trim().is_empty())
            }) {
