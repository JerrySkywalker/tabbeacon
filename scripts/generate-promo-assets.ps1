...
param(
    ...
    [string]$RunId = ("TB-V073-PROMO-" + (Get-Date -Format 'yyyyMMddHHmmss')),
    [string]$EvidenceRoot,
...
function Get-FrameManifestSha256([object[]]$Frames) {
    $lines = @($Frames | ForEach-Object { "{0}:{1}" -f $_.Name, (Get-Sha256 $_.FullName) })
...
function Read-FixtureReceipt([string]$Path, [string]$Head, [string]$FramesPath) {
    ...
    if ($fixtureData.source_sha -ne $Head -or $fixtureData.anchor_tab_match_count -ne 1 -or $fixtureData.target_window_match_count -ne 1 -or
        -not $fixtureData.real_windows_terminal -or -not $fixtureData.real_tabbeacon_renderer -or
        $fixtureData.real_model_session -or $fixtureData.desktop_capture -or -not $fixtureData.controlled_fixture_only -or
        $fixtureData.temporary_windows_created -ne 1 -or $fixtureData.temporary_windows_closed -ne 1 -or
        $fixtureData.owned_temporary_wt_remaining -ne 0 -or $fixtureData.owner_windows_closed -ne 0 -or
        $fixtureData.broad_windows_terminal_kill) {
    ...
    foreach ($frame in $frameFiles) {
        $dimensions = Get-ExactPngDimensions $frame.FullName
        if ($dimensions[0] -ne $fixtureData.frame_width -or $dimensions[1] -ne $fixtureData.frame_height) {
            throw "Frame dimensions do not match the exact-owned fixture receipt: $($frame.Name)"
        }
    }
    return [pscustomobject]@{
        Receipt = $fixtureData
        Frames = $frameFiles
        FixtureReceiptSha256 = Get-Sha256 $Path
        FrameManifestSha256 = Get-FrameManifestSha256 $frameFiles
    }
}

function Assert-PrivacyReview([string]$Path, [string]$Head, [object]$FixtureProof, [string]$FramesPath) {
    ...
    if ($privacy.source_sha -ne $Head -or $privacy.frame_count -ne $FixtureProof.Receipt.frame_count -or $privacy.review_disposition -ne 'PASS' -or
        $privacy.fixture_receipt_sha256 -ne $FixtureProof.FixtureReceiptSha256 -or $privacy.frame_manifest_sha256 -ne $FixtureProof.FrameManifestSha256) {
...
if ($Mode -eq 'Capture') {
    ...
    if ($dirtyPaths.Count -ne 0) { throw 'Promo capture requires a clean committed candidate.' }
...
$targetOutputHeight = 300
$cropHeight = [Math]::Min(
    [int]$fixtureProof.Receipt.frame_height,
    [int][Math]::Ceiling(([double]$fixtureProof.Receipt.frame_width * $targetOutputHeight) / 960)
)
$scaleFilter = "fps=10,crop=iw:${cropHeight}:0:0,scale=960:-2:flags=lanczos"
& $ffmpegTools.Ffmpeg -hide_banner -loglevel error -framerate 10 -i (Join-Path $frames 'frame-%04d.png') -vf "$scaleFilter,palettegen=stats_mode=diff" -n $palette
if ($LASTEXITCODE -ne 0) { throw "FFmpeg palette generation failed: $LASTEXITCODE" }
& $ffmpegTools.Ffmpeg -hide_banner -loglevel error -framerate 10 -i (Join-Path $frames 'frame-%04d.png') -i $palette -filter_complex "[0:v]$scaleFilter[x];[x][1:v]paletteuse=dither=bayer:bayer_scale=3:diff_mode=rectangle" -loop 0 -n $temporaryGif
if ($LASTEXITCODE -ne 0) { throw "FFmpeg GIF encoding failed: $LASTEXITCODE" }
& $ffmpegTools.Ffmpeg -hide_banner -loglevel error -i $temporaryGif -frames:v 1 -n $temporaryPoster
...
$receipt = [ordered]@{
    ...
    fixture_receipt_sha256 = $fixtureProof.FixtureReceiptSha256
    frame_manifest_sha256 = $fixtureProof.FrameManifestSha256
    fps = $gifFps
    ...
    anchor_tab_match_count = $fixtureProof.Receipt.anchor_tab_match_count
    ...
    temporary_windows_created = $fixtureProof.Receipt.temporary_windows_created
    temporary_windows_closed = $fixtureProof.Receipt.temporary_windows_closed
    owned_temporary_wt_remaining = $fixtureProof.Receipt.owned_temporary_wt_remaining
    owner_windows_closed = $fixtureProof.Receipt.owner_windows_closed
    broad_windows_terminal_kill = $fixtureProof.Receipt.broad_windows_terminal_kill
